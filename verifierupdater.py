...
def getRuns(user_id):
    allruns = utils.get_runs(processRuns, examiner=user_id)
    return allruns
...
for n, moderator in enumerate(moderators_data):
    print(n, moderator)
    if utils.is_user_deleted(examiner=moderator["id"]):
...
utils.make_lb("vdatabase.json", "runs_amount")
utils.make_lb("vdatabase.json", "game_amount")
